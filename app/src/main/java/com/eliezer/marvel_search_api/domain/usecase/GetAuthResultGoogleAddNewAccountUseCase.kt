...
class GetAuthResultGoogleAddNewAccountUseCase @Inject constructor(
    @IoDispatcher dispatcher: CoroutineDispatcher,
    private val firebaseRepository: FirebaseRepository
) : BaseFlowUseCase<Context, Result<AuthResult>>(dispatcher){
    override fun execute(params: Context): Flow<Result<AuthResult>> = firebaseRepository.signInAddGoogleNewAccount(params)
}